...
> **`test`-mode content OBSOLETE (2026-07-26):** `test` mode, `_TestTracker`, and `_TestDone` were
> removed with the mode itself (see [ADR-0007](0007-test-mode-owns-rollout-loop.md), also obsolete).
> **The "Render shutdown (play / random)" section below is STILL LIVE** — the named `RenderFinished`
> sentinel remains the shutdown mechanism for `play`/`random`/`--video` (`multigroup_environment.py`,
> `train_utils.py`). Read this ADR for that; ignore the `_TestTracker`/`_TestDone` framing.
