...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def __init__(
        self,
        xml_file: str = path.join(path.dirname(__file__), "assets", "ant.xml"),
        frame_skip: int = 5,
        ...
        **kwargs,
        ...
        self._random_morphology = random_morphology
        ...
        self._build_limb_mappings()
        self._save_model_template()
        self._limb_mask = np.ones(8, dtype=bool)
        self._leg_angles = self._leg_orig_angles.copy()
    ...
    def _get_reset_info(self):
        return {
            ...
            "limb_mask": self._limb_mask.astype(np.float64),
            "leg_angles": self._leg_angles.copy(),
        }
    ...
    # Joint names in qpos / obs order (mask index → joint name)
    _JOINT_NAMES = [
        "hip_1", "ankle_1",
        "hip_2", "ankle_2",
        "hip_3", "ankle_3",
    ...
    def _subtree(self, body_id: int) -> list[int]:
        result = [body_id]
        for child in range(self.model.nbody):
            if self.model.body_parentid[child] == body_id:
                result.extend(self._subtree(child))
        return result

    def _build_limb_mappings(self):
        skip = 2 * self._exclude_current_positions_from_observation
        qpos_obs_size = self.model.nq - skip

        self._jnt_ids = np.array(
            [self.model.joint(n).id for n in self._JOINT_NAMES], dtype=int
        )

        # actuator index for each joint (via actuator_trnid)
        self._act_ids = np.array(
            [int(np.where(self.model.actuator_trnid[:, 0] == jid)[0][0])
             for jid in self._jnt_ids],
            dtype=int,
        )

        # bodies and geoms to ghost per mask index
        self._ghost_body_ids: list[list[int]] = []
        self._ghost_geom_ids: list[list[int]] = []
        for jid in self._jnt_ids:
            joint_body = int(self.model.jnt_bodyid[jid])
            bodies = self._subtree(joint_body)
            # include parent body if it has no joints (geometry-carrier)
            parent = int(self.model.body_parentid[joint_body])
            if parent > 0 and not np.any(self.model.jnt_bodyid == parent):
                bodies.append(parent)
            self._ghost_body_ids.append(bodies)
            geoms: list[int] = []
            for b in bodies:
                start = int(self.model.body_geomadr[b])
                count = int(self.model.body_geomnum[b])
                geoms.extend(range(start, start + count))
            self._ghost_geom_ids.append(geoms)

        # obs indices for each joint's qpos/qvel entries
        self._obs_qpos_idx = np.array(
            [int(self.model.jnt_qposadr[jid]) - skip for jid in self._jnt_ids],
            dtype=int,
        )
        self._obs_qvel_idx = np.array(
            [qpos_obs_size + int(self.model.jnt_dofadr[jid]) for jid in self._jnt_ids],
            dtype=int,
        )

        # obs start indices for cfrc_ext entries per ghost body
        cfrc_obs_start = qpos_obs_size + self.model.nv
        self._obs_cfrc_starts: list[list[int]] = [
            [cfrc_obs_start + (b - 1) * 6 for b in bodies]
            for bodies in self._ghost_body_ids
        ]

        # upper_leg body IDs (geometry-carrier parent, last entry in hip ghost list)
        self._upper_leg_body_ids = [self._ghost_body_ids[2 * i][-1] for i in range(4)]
        # aux body IDs (hip joint body, first entry in hip ghost list)
        self._aux_body_ids = [self._ghost_body_ids[2 * i][0] for i in range(4)]
        # original leg angles in torso frame (radians)
        self._leg_orig_angles = np.array([
            np.arctan2(self.model.body_pos[aid][1], self.model.body_pos[aid][0])
            for aid in self._aux_body_ids
        ])

    def _save_model_template(self):
        self._tmpl_gear      = self.model.actuator_gear.copy()
        self._tmpl_mass      = self.model.body_mass.copy()
        self._tmpl_inertia   = self.model.body_inertia.copy()
        self._tmpl_contype   = self.model.geom_contype.copy()
        self._tmpl_conaffin  = self.model.geom_conaffinity.copy()
        self._tmpl_rgba      = self.model.geom_rgba.copy()
        self._tmpl_jnt_range = self.model.jnt_range.copy()
        self._tmpl_jnt_stiff = self.model.jnt_stiffness.copy()
        self._tmpl_body_quat = self.model.body_quat.copy()

    def _restore_model(self):
        self.model.actuator_gear[:]    = self._tmpl_gear
        self.model.body_mass[:]        = self._tmpl_mass
        self.model.body_inertia[:]     = self._tmpl_inertia
        self.model.geom_contype[:]     = self._tmpl_contype
        self.model.geom_conaffinity[:] = self._tmpl_conaffin
        self.model.geom_rgba[:]        = self._tmpl_rgba
        self.model.jnt_range[:]        = self._tmpl_jnt_range
        self.model.jnt_stiffness[:]    = self._tmpl_jnt_stiff
        self.model.body_quat[:]        = self._tmpl_body_quat

    def _compute_leg_angles(self, mask: np.ndarray) -> np.ndarray:
        active = mask[0::2]                          # hip bits → bool[4]
        angles = self._leg_orig_angles.copy()
        if active.all():
            return angles
        idx = np.where(active)[0]
        orig = self._leg_orig_angles[idx]
        centroid = np.arctan2(np.sin(orig).mean(), np.cos(orig).mean())
        n = len(idx)
        spacing = 2 * np.pi / n
        for k, i in enumerate(idx):
            angles[i] = centroid + (k - (n - 1) / 2) * spacing
        return angles

    def _rotate_leg(self, leg_idx: int, new_angle: float):
        delta = new_angle - self._leg_orig_angles[leg_idx]
        h = delta / 2
        self.model.body_quat[self._upper_leg_body_ids[leg_idx]] = [
            np.cos(h), 0.0, 0.0, np.sin(h)  # w, x, y, z (rotation around z)
        ]

    def _validate_mask(self, mask: np.ndarray):
        for hip, ankle in [(0, 1), (2, 3), (4, 5), (6, 7)]:
            if not mask[hip] and mask[ankle]:
                raise ValueError(
                    f"{self._JOINT_NAMES[ankle]} enabled but "
                    f"{self._JOINT_NAMES[hip]} disabled"
                )
        if mask.sum() < 4:
            raise ValueError(f"mask must have >=4 active joints, got {int(mask.sum())}")

    def _ghost_joint(self, i: int):
        jid      = self._jnt_ids[i]
        aid      = self._act_ids[i]
        bodies   = self._ghost_body_ids[i]
        geoms    = self._ghost_geom_ids[i]

        self.model.actuator_gear[aid] = 0
        for b in bodies:
            self.model.body_mass[b]        = 1e-10
            self.model.body_inertia[b]     = 1e-10
        for g in geoms:
            self.model.geom_contype[g]     = 0
            self.model.geom_conaffinity[g] = 0
            self.model.geom_rgba[g, 3]     = 0.0
        self.model.jnt_range[jid]    = [0.0, 0.0]
        self.model.jnt_stiffness[jid] = 1e4

    def reset(self, *, seed=None, options=None):
        if self._random_morphology and (not options or "limb_mask" not in options):
            mask = sample_valid_mask()
        elif options and "limb_mask" in options:
            mask = np.asarray(options["limb_mask"], dtype=bool)
        else:
            mask = np.ones(8, dtype=bool)
        self._validate_mask(mask)
        self._limb_mask = mask
        self._restore_model()
        for i in range(8):
            if not mask[i]:
                self._ghost_joint(i)
        self._leg_angles = self._compute_leg_angles(mask)
        for i, ang in enumerate(self._leg_angles):
            if mask[2 * i]:
                self._rotate_leg(i, ang)
    ...
    def reset_model(self):
        ...
        for i in range(8):
            if not self._limb_mask[i]:
                qpos[self.model.jnt_qposadr[self._jnt_ids[i]]] = 0.0
    ...
    def _get_obs(self):
        ...
        if not self._limb_mask.all():
            for i in range(8):
                if not self._limb_mask[i]:
                    obs[self._obs_qpos_idx[i]] = 0.0
                    obs[self._obs_qvel_idx[i]] = 0.0
                    if self._include_cfrc_ext_in_observation:
                        for start in self._obs_cfrc_starts[i]:
                            obs[start:start + 6] = 0.0
        return obs
