...
**Proxy run** (tuning):
The short run each Optuna trial executes, standing in for the longer deployment run, with checkpoint writes off (the tuner reads TB only). A short-horizon signal, and each study has its own: the PPG proxy is `max_epochs=500` against a 1500-epoch deployment (`configs/ppo_ant_ppg_tune.yaml`), where the morphology [resample](../envs/CONTEXT.md) fires only once (~epoch 313, period ≈ `resample_interval × max_episode_length ÷ horizon_length`) versus ~4 at deployment length; the codesign proxy is `max_epochs=2000` against 3000 (`configs/ppo_ant_codesign_tune.yaml`), giving ~32 resample windows versus ~48. Fidelity is the number of windows the proxy spans — the fewer, the less the tuned winner transfers.
...
**Screen** / **focus** (tuning):
The two stages of a sweep. A **screen** searches many parameters thinly to rank which ones matter; a **focus** sweep then searches only those, densely. Staged narrow sweeps that freeze a winner per stage (the `3a`/`3b`/`3c` comments in the codesign configs) are the older, greedy alternative — they cannot see interactions between parameters fixed in different stages.
