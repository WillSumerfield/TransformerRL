"""PPO training on CustomAnt-v5 w/ the per-joint LegTransformer."""
import argparse
import datetime
import functools
import sys
...
def build(cfg: dict, device: torch.device, seed: int):
    ...
    print(f"Curriculum: p(random morphology)=0 until {onset_start} steps, "
          f"then ramps to 1 by {onset_end} steps.")
    ...
    wrappers = [
        ...
        functools.partial(CurriculumWrapper,
                          ...
                          num_envs=ec["num_envs"]),
    ]
    venvs = gym.make_vec(ec["id"], num_envs=ec["num_envs"],
                         vectorization_mode="async", wrappers=wrappers)
