...
The **semantic** kind a module token carries. The vocabulary mirrors a **real modular-robot hardware library** (the eventual real task assembles bodies from such physical modules — that grounding is the *why*; the Phase-5 types are generic **placeholders** until the real catalog lands). **Effector** — an actuated module (a pivot/motor). Phase 5 makes its **type** (joint axis / range, and eventually DOF-count) a **generator choice per module** — replacing today's depth-forced swing(eff0)/knee(eff1) assignment. **Cap** — a **terminal** module. Phase 5 gives it **morphology** (feet, grippers, …) but keeps it **passive — no actions for now**; **stop** = a morphology-less Cap that ends a limb and adds nothing. **Connector** — the **interface between modules**: **primarily a geometric mounting offset**, plus a **compatibility mask** (which module type may attach next); a semantic pre-marker before each module, materialized as a real token and (by 5b's end) an **independent physical module** — superseding ADR-0014's `Connector → none`. **start** = a special (base) Connector. **Link** — a passive extension module (no motor); **deferred out of Phase 5** to a later **"Links & Parameterized Modules"** phase (which also makes per-module params — link length, motor range — generator-designed). Only Effector (+ the implicit start/stop) exists before Phase 5; the rest are reserved. See [ADR-0014](../docs/adr/0014-generalized-construction-vocabulary.md).
