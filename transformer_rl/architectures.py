...
from .tokenize import tokenize_4, tokenize_8, ROOT_DIM, HIP_DIM, ANKLE_DIM, HIP_DIM_8, ANKLE_DIM_8

_TOKENIZE = {4: tokenize_4, 8: tokenize_8}

# codesign token type / mode ids (see CONTEXT.md "Codesign tokens")
_T_ROOT, _T_HIP, _T_ANKLE, _T_START = 0, 1, 2, 3
...
class LimbTransformer(nn.Module):
    def __init__(
        ...
        if codesign_tokens:
            ...
            type_ids = torch.tensor(
                [_T_ROOT] + [_T_START] * n_limbs + [_T_HIP] * n_limbs + [_T_ANKLE] * n_limbs,
                dtype=torch.long)
    ...
    def _encode_legacy(self, root, hip_tok, ankle_tok, active_mask, B):
        ...
        # (B, 1+2*n_limbs, 1): root always active, then hip masks, then ankle masks
    ...
    def _encode_codesign(self, root, hip_tok, ankle_tok, active_mask, B):
    ...
    def codesign_forward(self, obs: torch.Tensor):
        ...
        root, hip_tok, ankle_tok, active_mask = self.tokenize_fn(obs)
        H = self._encode_codesign(root, hip_tok, ankle_tok, active_mask, obs.shape[0])
    ...
    def forward(self, obs: torch.Tensor, compute_value: bool = True,
        ...
        if self.codesign_tokens:
            x = self._encode_codesign(root, hip_tok, ankle_tok, active_mask, B)
        else:
            x = self._encode_legacy(root, hip_tok, ankle_tok, active_mask, B)
